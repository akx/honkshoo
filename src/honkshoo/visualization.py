...
def plot_channels(db: sqlite3.Connection, *, channels: list[str] | None):
    ...
    for channel in channels:
        try:
            ...
            log.warning(f"Channel {channel} not found; available channels: {', '.join(ch_map)}")
            continue
        res = [
        ...
        if not res:
            log.warning(f"Channel {channel} has no data")
            continue
        val0 = res[0][1]
        if all(value == val0 for (_, value) in res):
            log.warning(f"Channel {channel} is constant over {len(res)} points: {val0}, not plotting")
            continue
        data = np.array(res)
